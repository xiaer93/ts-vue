...
Vue分为运行版和完整版，完整版本包含compile模块，对如`<div id="app">{{message}}</div>`的模版语法进行了编译。为了简化理解逻辑，笔者重构时直接将compile模块忽略了，全部通过渲染函数进行渲染。
...
谈到三大框架，必定要谈虚拟DOM。通过访问`vm._vnode`可以查看vue虚拟DOM的结构，其通过children属性构建树形结构。

图片

为什么要定义虚拟DOM？怎么定义虚拟DOM？虚拟DOM有什么好处？推荐阅读[snabbdom](https://github.com/snabbdom/snabbdom/blob/master/src/snabbdom.ts)开源库，snabbdom的核心非常精简，总共318行，最关键一点，Vue的虚拟DOM是参考它改进的。


2. Proxy和Reflect使用

谈到Vue的特性，必定要了解数据驱动和响应式。通过访问`app._data`可以查看经过响应式处理后的data结构。

图片

Vue源码使用Object.defineProperty，笔者将使用Proxy对象进行属性拦截，直接拦截整个对象，如下述代码，实例化后通过`p.name`可以访问到`this.data={name: 'xiaoming'}`。
...
看下述代码，在定时器中修改`name`和`message`属性，可以发现视图更新了。但是，runCount此时的值是几？如果我们同步修改更多的属性，会影响runCount的值吗？
...
Vue源码对此进行进行了优化，Vue的每次更新都是异步更新，同时业务中常常会使用到$nextTick。了解Js的事件循环，有助于理解Vue的更新的原理。
...
观察下面的demo，当用户点击按钮是，点击次数加1，同时控制台输出`customClick click`。
...
不仅支持自定义组件，也内置了`transition`/`keep-alive`等组件，Vue是如何实现组件功能的？父子组件如何进行消息传递？（props、on）
...
观察下面demo，用户首先看不到任何文字，1秒钟之后观看到UI变化`111&222&333`。

不仅支持内置指令，也允许用户自定义指令。指令代码是如何控制UI的？

ps:有次业务需要在php系统中开发新的需求，新的项目使用vue进行开发，新项目以iframe形式被嵌入在老页面中。iframe技术非常有效，但是我们发现新项目中的弹窗无法全屏展示（半透明mask无法全屏），后台借助指令实现。推荐开源库https://github.com/calebroseland/vue-dom-portal。
...
笔者主要学习均来黄老师的两套课程，在次着重推荐。文章有些demo来自黄老师，不知道是否侵权。（如有侵权必定删除重新补充）
